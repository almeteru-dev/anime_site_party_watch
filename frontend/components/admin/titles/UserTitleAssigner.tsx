...
export function UserTitleAssigner() {
	const [query, setQuery] = useState("")
	const [userResults, setUserResults] = useState<AdminUser[] | null>(null)
	const [selectedUser, setSelectedUser] = useState<AdminUser | null>(null)

	const [all, setAll] = useState<Title[] | null>(null)
	const [assigned, setAssigned] = useState<Title[] | null>(null)
	...
	const loadAll = useCallback(async () => {
		try {
			const items = await adminListTitles()
	...
	const loadAssigned = useCallback(async (userId: number) => {
		try {
			const items = await adminGetUserTitles({ user_id: userId })
			setAssigned(sortById(items))
		} catch (e: any) {
			setError(e?.message || "Failed to load user titles")
			setAssigned([])
		}
	}, [])
	...
	const available = useMemo(() => {
		if (!all) return null
		return all.filter((t) => !assignedIds.has(t.id))
	}, [all, assignedIds])
	...
	const assign = useCallback(
		async (titleId: number) => {
			...
			setError(null)
			try {
				await adminAssignTitleToUser({ user_id: selectedUser.id, title_id: titleId })
				await loadAssigned(selectedUser.id)
			} catch (e: any) {
				setError(e?.message || "Failed to assign title")
			} finally {
				setIsBusy(false)
			}
		},
	...
	const unassign = useCallback(
		async (titleId: number) => {
			...
			setError(null)
			try {
				await adminUnassignTitleFromUser({ user_id: selectedUser.id, title_id: titleId })
				await loadAssigned(selectedUser.id)
			} catch (e: any) {
				setError(e?.message || "Failed to unassign title")
			} finally {
				setIsBusy(false)
			}
		},
	...
	const assignedContent = useMemo(() => {
		...
		if (assigned.length === 0) return <div className="text-sm text-foreground-muted">No titles assigned.</div>
		return assigned.map((t) => (
			<div key={t.id} className="flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-background px-4 py-3">
				<div className="min-w-0">
					<div className="text-sm font-semibold text-foreground truncate">{t.name_en}</div>
					<div className="text-xs text-foreground-muted truncate">{t.code}</div>
				</div>
				<button
	...
	const availableContent = useMemo(() => {
		...
		if (available.length === 0) return <div className="text-sm text-foreground-muted">No available titles.</div>
		...
		<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
			...
			<div className="rounded-2xl border border-border/60 bg-background-secondary/40">
				<div className="px-5 py-4 border-b border-border/50">
					...
					<div className="text-xs text-foreground-muted mt-1">Назначенные и доступные звания</div>
				</div>
				<div className="p-5">
					{!selectedUser ? (
						...
						<div className="grid grid-cols-1 gap-6">
							<div>
								...
								<div className="mt-3 space-y-2">{assignedContent}</div>
							</div>
							<div>
								<div className="text-xs font-semibold text-foreground-muted uppercase tracking-wider">Available</div>
								<div className="mt-3 space-y-2">{availableContent}</div>
							</div>
						</div>
					)}
				</div>
